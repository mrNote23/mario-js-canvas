...
const init = () => {
  ...
  genericObjects = [new GenericObject({ x: -1, y: -1, image: backgroundImage }), new GenericObject({ x: -1, y: -1, image: hillsImage })]
...
function animate() {
  ...
  // изменение спрайта
  if (keys.right.pressed && lastKey === "right" && player.currentSrpite !== player.sprites.run.right) {
    player.frames = 1
    player.currentSrpite = player.sprites.run.right
    player.currentCropWidth = player.sprites.run.cropWidth
    player.width = player.sprites.run.width
  } else if (keys.left.pressed && lastKey === "left" && player.currentSrpite !== player.sprites.run.left) {
    player.currentSrpite = player.sprites.run.left
    player.currentCropWidth = player.sprites.run.cropWidth
    player.width = player.sprites.run.width
  } else if (!keys.left.pressed && lastKey === "left" && player.currentSrpite !== player.sprites.stand.left) {
    player.currentSrpite = player.sprites.stand.left
    player.currentCropWidth = player.sprites.stand.cropWidth
    player.width = player.sprites.stand.width
  } else if (!keys.right.pressed && lastKey === "right" && player.currentSrpite !== player.sprites.stand.right) {
